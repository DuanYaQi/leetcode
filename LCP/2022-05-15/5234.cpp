...
public:
    ...
    public:
        ...
        bool operator== (charMap &others) {
            for (int i = 0; i < 26; ++i) {
                if (nums[i] != others.nums[i]) return false;
            }
            return true;
        }
        ...
        bool operator!= (charMap &others) {
            for (int i = 0; i < 26; ++i) {
                if (nums[i] != others.nums[i]) return true;
            }
            return false;
        }
    };
    ...
    vector<string> removeAnagrams(vector<string>& words) {
        ...
        charMap cmap;
        for (auto c : words[0]) cmap[c]++;
        ans.push_back(words[0]);
        
        
        for (int i = 1; i < n; ++i) {
            charMap newmap;
            for (auto c : words[i]) newmap[c]++;
            if (cmap != newmap)  ans.push_back(words[i]);
            cmap = newmap;
        }
