...
令 `P[i]=nums[0]+nums[1]+…+nums[i]`。那么每个连续子数组的和 `sum(i, j)` 就可以写成 `P[j] - P[i-1]` 其中 0 < i < j 的形式。此时，判断子数组的和能否被 k 整除就等价于判断 `(P[j] - P[i-1]) mod k == 0`，根据**同余定理**，只要 `P[j] mod k == P[i-1] mod k`，就可以保证上面的等式成立。

遍历到第 i 个元素时，统计以 i 结尾的符合条件的子数组个数。维护一个以前缀和模 k 的值为键，出现次数为值的哈希表 record，在遍历的同时进行更新。这样在计算以 i 结尾的符合条件的子数组个数时，根据上面的分析，答案即为 $[0..i-1]$ 中前缀和模 k 也为 $P[i] \bmod k$ 的位置个数，即 $\textit{record}[P[i] \bmod k]$。
